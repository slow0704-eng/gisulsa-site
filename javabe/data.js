...
CPPG.mcq = [
  ...
  // ───────── 보강 1. Java · JVM ─────────
  { s:'s1', d:1, q:'오버로딩과 오버라이딩의 결정 시점을 바르게 짝지은 것은?', c:['오버로딩 런타임 · 오버라이딩 컴파일 시점','오버로딩 컴파일 시점 · 오버라이딩 런타임','둘 다 컴파일 시점','둘 다 런타임'], a:1, e:'오버로딩은 정적 바인딩, 오버라이딩은 동적 바인딩이다.' },
  { s:'s1', d:2, q:'추상 클래스 대신 인터페이스를 선택하는 기준으로 가장 적절한 것은?', c:['공통 상태(필드)를 공유해야 할 때','공통 구현 코드를 물려주고 싶을 때','서로 무관한 타입에 같은 능력을 부여할 때','생성자를 물려주고 싶을 때'], a:2, e:'인터페이스는 can-do, 추상 클래스는 is-a 관계에 가깝다.' },
  { s:'s1', d:1, q:'컴파일러가 처리(try-catch 또는 throws)를 강제하는 예외는?', c:['RuntimeException','Error','체크드 예외','NullPointerException'], a:2, e:'언체크드는 강제하지 않으며 프로그래밍 오류에 해당한다.' },
  { s:'s1', d:2, q:'for-each 로 순회하면서 컬렉션의 원소를 직접 제거하면 발생하는 예외는?', c:['IllegalStateException','ConcurrentModificationException','UnsupportedOperationException','IndexOutOfBoundsException'], a:1, e:'Iterator.remove() 나 removeIf() 를 사용해야 한다.' },
  { s:'s1', d:2, q:'ConcurrentHashMap 이 Hashtable 보다 빠른 이유는?', c:['동기화를 아예 하지 않아서','버킷 단위로 락을 나눠 잡아서','읽기 전용이라서','키를 정렬해서'], a:1, e:'Hashtable 은 전체를 동기화하는 레거시 클래스다.' },
  { s:'s1', d:1, q:'입력 순서를 유지하면서 키-값을 저장하는 표준 컬렉션은?', c:['HashMap','LinkedHashMap','TreeMap','Hashtable'], a:1, e:'accessOrder 를 켜면 LRU 캐시로도 활용한다.' },
  { s:'s1', d:2, q:'제네릭의 타입 소거(Type Erasure)에 대한 설명으로 옳은 것은?', c:['런타임에도 제네릭 타입 정보가 유지된다','컴파일 후 타입 정보가 지워진다','성능 향상을 위한 JIT 기능이다','리플렉션으로 항상 복원할 수 있다'], a:1, e:'그래서 new T() 같은 코드가 불가능하다.' },
  { s:'s1', d:3, q:'PECS 원칙에서 컬렉션에 값을 넣기만 할 때 사용하는 와일드카드는?', c:['? extends T','? super T','?','T'], a:1, e:'Producer-Extends, Consumer-Super 로 기억한다.' },
  { s:'s1', d:2, q:'클래스 로더의 위임 모델 순서로 옳은 것은?', c:['Application → Platform → Bootstrap','Bootstrap → Platform → Application','Platform → Bootstrap → Application','순서는 정해져 있지 않다'], a:1, e:'상위 로더에 먼저 위임해 중복 로딩과 변조를 막는다.' },
  { s:'s1', d:2, q:'JVM 실행 엔진에서 JIT 컴파일러의 역할은?', c:['소스를 바이트코드로 변환','자주 실행되는 바이트코드를 기계어로 변환','클래스를 메모리에 적재','가비지 컬렉션 수행'], a:1, e:'인터프리터의 느린 반복 실행을 보완한다.' },
  { s:'s1', d:2, q:'Minor GC 와 Full GC 의 차이로 옳은 것은?', c:['Minor 는 Old 영역을 수집한다','Full GC 는 Stop-The-World 영향이 크다','Minor GC 는 실행되지 않는 경우가 많다','Full GC 가 항상 더 빠르다'], a:1, e:'Minor 는 Young 대상으로 짧고 잦게 일어난다.' },
  { s:'s1', d:3, q:'AtomicInteger 가 락 없이 원자성을 보장하는 방식은?', c:['synchronized 내부 사용','CAS(비교 후 교환) 재시도','volatile 만으로 보장','스레드 우선순위 조정'], a:1, e:'경합이 심하면 재시도가 늘어 오히려 비용이 커질 수 있다.' },
  { s:'s1', d:2, q:'교착 상태의 4가지 조건에 해당하지 않는 것은?', c:['상호배제','점유와 대기','비선점','우선순위 역전'], a:3, e:'네 번째 조건은 환형 대기다.' },
  { s:'s1', d:3, q:'I/O 대기가 많은 작업의 스레드 풀 크기를 정하는 기준으로 가장 적절한 것은?', c:['코어 수와 동일하게','대기 비율을 반영해 코어 수보다 크게','항상 1개','메모리 크기에 비례해'], a:1, e:'CPU 바운드는 코어 수 근처이며 어느 쪽이든 실측 후 조정이 원칙이다.' },
  { s:'s1', d:1, q:'멀티스레드 환경에서 문자열을 이어 붙일 때 안전한 클래스는?', c:['String','StringBuilder','StringBuffer','StringJoiner'], a:2, e:'StringBuffer 는 동기화되어 있고 StringBuilder 는 비동기라 빠르다.' },
  { s:'s1', d:3, q:'기본값을 만드는 비용이 클 때 Optional 에서 권장되는 메서드는?', c:['orElse','orElseGet','get','orElseThrow'], a:1, e:'orElse 의 인자는 값이 있어도 항상 평가된다.' },
  { s:'s1', d:2, q:'Java 21 가상 스레드가 가장 큰 이점을 주는 작업 유형은?', c:['CPU 집약 연산','I/O 대기가 많은 요청 처리','정렬 알고리즘','GC 튜닝'], a:1, e:'블로킹 대기 중 캐리어 스레드를 반납해 처리량이 올라간다.' },
  { s:'s1', d:1, q:'Java 17 의 record 가 주로 대체하는 것은?', c:['열거형','값만 담는 불변 DTO·VO 클래스','인터페이스','제네릭'], a:1, e:'생성자·접근자·equals·hashCode·toString 이 자동 생성된다.' },

  // ───────── 보강 2. Spring · Spring Boot ─────────
  { s:'s2', d:1, q:'IoC(제어의 역전)를 가장 잘 설명한 것은?', c:['개발자가 객체를 직접 생성한다','객체 생성과 생명주기 제어권을 컨테이너가 가진다','상속으로 기능을 확장한다','예외 처리를 위임한다'], a:1, e:'DI 는 IoC 를 구현하는 방법 중 하나다.' },
  { s:'s2', d:2, q:'필드 주입(@Autowired)을 지양하는 이유로 가장 적절한 것은?', c:['런타임 성능이 느려서','final 을 쓸 수 없고 프레임워크 없이 테스트하기 어려워서','컴파일 오류가 나서','빈이 두 번 생성되어서'], a:1, e:'생성자 주입은 불변과 필수 의존을 드러낸다.' },
  { s:'s2', d:2, q:'프로토타입 스코프 빈에 대한 설명으로 옳은 것은?', c:['컨테이너가 소멸 콜백까지 관리한다','요청할 때마다 새 인스턴스가 생성된다','싱글턴보다 항상 빠르다','웹 환경에서만 사용할 수 있다'], a:1, e:'생성 이후의 관리는 사용하는 쪽 책임이다.' },
  { s:'s2', d:2, q:'같은 타입의 빈이 여러 개일 때 특정 빈을 이름으로 지정하는 애너테이션은?', c:['@Primary','@Qualifier','@Lazy','@Scope'], a:1, e:'@Primary 는 이름 지정 없이 기본 후보를 정한다.' },
  { s:'s2', d:2, q:'@PostConstruct 가 호출되는 시점은?', c:['빈 인스턴스 생성 직전','의존관계 주입이 끝난 직후','컨테이너 종료 직전','첫 요청이 들어올 때'], a:1, e:'생성 → 주입 → 초기화 콜백 → 사용 → 소멸 순이다.' },
  { s:'s2', d:3, q:'스프링 부트 2.6 이상에서 순환 참조가 발생했을 때 가장 바람직한 해결은?', c:['allow-circular-references 를 켠다','@Lazy 를 붙인다','공통 로직 분리나 이벤트로 설계를 바꾼다','필드 주입으로 변경한다'], a:2, e:'설정이나 @Lazy 는 임시방편이며 기본은 순환 참조 금지다.' },
  { s:'s2', d:2, q:'스프링 AOP 에서 JDK 동적 프록시가 사용되는 조건은?', c:['대상 클래스가 final 일 때','대상이 인터페이스를 구현했을 때','메서드가 private 일 때','빈 스코프가 프로토타입일 때'], a:1, e:'인터페이스가 없으면 CGLIB 이 사용되며 부트 기본값도 CGLIB 이다.' },
  { s:'s2', d:3, q:'같은 클래스 안에서 this 로 호출할 때 함께 동작하지 않는 애너테이션으로만 묶인 것은?', c:['@Transactional, @Cacheable, @Async','@Component, @Service','@GetMapping, @PostMapping','@Valid, @NotNull'], a:0, e:'모두 프록시를 거쳐야 적용되는 기능이다.' },
  { s:'s2', d:2, q:'private 메서드에 @Transactional 을 붙였을 때의 결과는?', c:['정상 동작한다','프록시를 적용할 수 없어 무시된다','컴파일 오류가 발생한다','읽기 전용으로 동작한다'], a:1, e:'public 메서드로 분리하거나 다른 빈으로 옮긴다.' },
  { s:'s2', d:2, q:'상위 트랜잭션이 반드시 존재해야 하고 없으면 예외를 던지는 전파 속성은?', c:['REQUIRED','MANDATORY','SUPPORTS','NEVER'], a:1, e:'NEVER 는 반대로 트랜잭션이 있으면 예외를 던진다.' },
  { s:'s2', d:3, q:'세이브포인트를 이용해 부분 롤백을 지원하는 전파 속성은?', c:['NESTED','REQUIRES_NEW','NOT_SUPPORTED','SUPPORTS'], a:0, e:'JDBC 드라이버에 의존적이라 사용 전 제약을 확인해야 한다.' },
  { s:'s2', d:2, q:'모든 컨트롤러의 예외를 한곳에서 처리해 응답 포맷을 통일하는 방법은?', c:['@ControllerAdvice·@RestControllerAdvice','필터에서 try-catch','컨트롤러마다 try-catch','인터셉터 preHandle'], a:0, e:'@ExceptionHandler 와 함께 공통 에러 응답을 만든다.' },
  { s:'s2', d:2, q:'JPA 리포지터리 계층만 임베디드 DB 로 빠르게 검증하는 슬라이스 테스트 애너테이션은?', c:['@SpringBootTest','@WebMvcTest','@DataJpaTest','@MockBean'], a:2, e:'테스트 범위를 좁혀야 빌드 시간이 유지된다.' },
  { s:'s2', d:3, q:'운영 환경에서 Actuator 를 사용할 때 가장 중요한 조치는?', c:['모든 엔드포인트를 공개한다','노출 범위를 제한하고 인증을 건다','health 만 끈다','포트를 8080 으로 고정한다'], a:1, e:'인증 없이 열면 운영 정보가 그대로 노출된다.' },
  { s:'s2', d:1, q:'@RestController 와 동일한 조합은?', c:['@Controller + @ResponseBody','@Controller + @RequestMapping','@Component + @Bean','@Service + @ResponseBody'], a:0, e:'반환값이 뷰 이름이 아니라 응답 본문으로 처리된다.' },
  { s:'s2', d:2, q:'스프링 부트 3.x 로 올릴 때 반드시 확인해야 하는 변화는?', c:['Java 8 지원 강화','javax 에서 jakarta 로 네임스페이스 변경','XML 설정 필수화','내장 서버 제거'], a:1, e:'Java 17 이상이 필수이며 의존 라이브러리 호환성도 함께 본다.' },

  // ───────── 보강 3. JPA · 데이터베이스 ─────────
  { s:'s3', d:1, q:'JPA 엔티티의 생명주기 상태에 해당하지 않는 것은?', c:['비영속','영속','준영속','병합'], a:3, e:'상태는 비영속·영속·준영속·삭제 네 가지다.' },
  { s:'s3', d:2, q:'준영속 상태의 엔티티 필드를 수정했을 때 일어나는 일은?', c:['커밋 시 UPDATE 가 실행된다','변경 감지가 동작하지 않아 반영되지 않는다','예외가 발생한다','즉시 INSERT 된다'], a:1, e:'다시 조회해 수정하거나 merge 를 사용해야 한다.' },
  { s:'s3', d:3, q:'mappedBy 로 지정된 쪽의 @OneToOne 에서 지연 로딩이 동작하지 않는 이유는?', c:['JPA 표준이 금지해서','외래 키가 없어 대상 존재 여부를 확인해야 해서','프록시를 지원하지 않아서','캐시 때문에'], a:1, e:'널 여부를 알아야 프록시를 만들 수 있어 결국 조회가 발생한다.' },
  { s:'s3', d:2, q:'cascade = ALL 을 사용할 때의 원칙으로 옳은 것은?', c:['모든 연관관계에 기본 적용한다','부모가 자식을 단독 소유할 때만 사용한다','양방향에서만 사용한다','조회 성능을 위해 사용한다'], a:1, e:'공유되는 엔티티에 걸면 의도치 않은 연쇄 삭제가 발생한다.' },
  { s:'s3', d:2, q:'기본 키 생성 전략 IDENTITY 의 특징으로 옳은 것은?', c:['시퀀스를 미리 할당해 성능이 좋다','persist 시점에 즉시 INSERT 되어 쓰기 지연이 사라진다','키 테이블을 사용한다','MySQL 에서 사용할 수 없다'], a:1, e:'SEQUENCE 는 allocationSize 로 호출 횟수를 줄일 수 있다.' },
  { s:'s3', d:2, q:'동적 쿼리를 타입 안전하게 작성하기 위해 실무에서 선호되는 것은?', c:['문자열 JPQL','QueryDSL','네이티브 쿼리','스토어드 프로시저'], a:1, e:'JPQL 은 문자열이라 컴파일 시점에 오류를 잡지 못한다.' },
  { s:'s3', d:3, q:'1:N 컬렉션 fetch join 에서 결과 행이 늘어나는 문제의 전통적 해결책은?', c:['distinct 사용','LIMIT 추가','EAGER 로 변경','인덱스 추가'], a:0, e:'하이버네이트 6부터는 기본적으로 중복을 제거한다.' },
  { s:'s3', d:2, q:'조회하려는 컬럼이 모두 인덱스에 포함되어 테이블 접근을 생략하는 것은?', c:['클러스터드 인덱스','커버링 인덱스','유니크 인덱스','함수 기반 인덱스'], a:1, e:'조회 성능이 크게 개선되지만 인덱스 크기는 커진다.' },
  { s:'s3', d:2, q:'EXPLAIN 결과에서 성능 경고 신호로 보아야 하는 것은?', c:['type = const','type = ALL','type = ref','rows = 1'], a:1, e:'Using filesort·Using temporary 도 함께 확인한다.' },
  { s:'s3', d:2, q:'인덱스 효율이 가장 좋은 컬럼은?', c:['카디널리티가 높은 컬럼','값이 두 종류뿐인 컬럼','자주 갱신되는 컬럼','NULL 이 대부분인 컬럼'], a:0, e:'선택도가 높아야 스캔 범위를 크게 줄인다.' },
  { s:'s3', d:3, q:'커넥션 풀 크기를 계속 늘렸는데 오히려 응답이 느려졌다. 가장 타당한 설명은?', c:['풀 크기는 성능과 무관하다','DB 의 코어·디스크가 병목이라 경합만 늘어난다','JVM 힙이 부족해서다','네트워크 대역폭 때문이다'], a:1, e:'적정 크기를 실측으로 찾는 것이 원칙이다.' },
  { s:'s3', d:2, q:'반정규화를 적용할 때의 기준으로 가장 적절한 것은?', c:['항상 조회 성능이 좋으므로 기본 적용','측정된 병목과 정합성 관리 비용을 근거로 제한적 적용','정규화보다 쉬우므로 우선 적용','저장 공간 절약을 위해 적용'], a:1, e:'중복이 늘어 정합성 관리 비용이 발생한다.' },
  { s:'s3', d:1, q:'트랜잭션의 ACID 에 해당하지 않는 것은?', c:['원자성','일관성','격리성','확장성'], a:3, e:'네 번째는 지속성(Durability)이다.' },

  // ───────── 보강 4. 웹 · 네트워크 · 보안 ─────────
  { s:'s4', d:1, q:'TCP 와 UDP 의 차이로 옳은 것은?', c:['UDP 는 연결을 수립하고 순서를 보장한다','TCP 는 흐름·혼잡 제어를 제공한다','TCP 가 항상 더 빠르다','UDP 는 재전송을 보장한다'], a:1, e:'UDP 는 비연결이라 DNS·실시간 스트리밍에 쓰인다.' },
  { s:'s4', d:2, q:'TCP 3-way 핸드셰이크의 순서로 옳은 것은?', c:['SYN → ACK → FIN','SYN → SYN+ACK → ACK','FIN → ACK → FIN','ACK → SYN → SYN+ACK'], a:1, e:'연결 종료는 FIN → ACK → FIN → ACK 의 4-way 다.' },
  { s:'s4', d:2, q:'HTTP/2 가 1.1 대비 개선한 항목으로 거리가 먼 것은?', c:['멀티플렉싱','헤더 압축','바이너리 프레이밍','UDP 기반 전송'], a:3, e:'UDP(QUIC) 기반은 HTTP/3 의 특징이다.' },
  { s:'s4', d:2, q:'ETag 와 If-None-Match 를 사용한 조건부 요청의 응답 코드는?', c:['200','204','304','404'], a:2, e:'변경이 없으면 본문 없이 304 로 응답해 트래픽을 줄인다.' },
  { s:'s4', d:2, q:'CSRF 완화를 위해 쿠키에 설정하는 속성은?', c:['HttpOnly','Secure','SameSite','Max-Age'], a:2, e:'HttpOnly 는 XSS, Secure 는 평문 전송을 막는 속성이다.' },
  { s:'s4', d:3, q:'공개 클라이언트(모바일·SPA)의 OAuth 2.0 권장 플로우는?', c:['Implicit','Authorization Code + PKCE','Resource Owner Password','Client Credentials'], a:1, e:'Implicit 은 토큰 노출 위험으로 폐기가 권장된다.' },
  { s:'s4', d:2, q:'Spring Security 가 인증 정보를 보관하는 SecurityContextHolder 의 기본 저장 방식은?', c:['세션 직접 저장','ThreadLocal','정적 필드','Redis'], a:1, e:'비동기·스레드 전환 시 컨텍스트 전파에 주의해야 한다.' },
  { s:'s4', d:2, q:'서버가 공격자가 지정한 내부 주소로 요청을 보내게 되는 취약점은?', c:['XSS','CSRF','SSRF','SQL 인젝션'], a:2, e:'URL 화이트리스트와 메타데이터 주소 차단으로 대응한다.' },
  { s:'s4', d:2, q:'REST API URI 설계로 가장 적절한 것은?', c:['GET /getUserOrders?id=1','GET /users/1/orders?status=PAID','POST /orders/delete/1','GET /order/getList'], a:1, e:'자원은 명사·복수형, 행위는 HTTP 메서드로 표현한다.' },
  { s:'s4', d:1, q:'클라이언트가 허용된 호출 횟수를 초과했을 때 반환하는 상태 코드는?', c:['409','422','429','503'], a:2, e:'429 와 함께 재시도 안내 헤더를 주는 것이 좋다.' },

  // ───────── 보강 5. 아키텍처 · 설계 ─────────
  { s:'s5', d:2, q:'클래스가 변경되어야 하는 이유가 하나여야 한다는 원칙은?', c:['단일 책임 원칙','개방-폐쇄 원칙','인터페이스 분리 원칙','의존 역전 원칙'], a:0, e:'책임이 섞이면 변경이 연쇄적으로 번진다.' },
  { s:'s5', d:3, q:'하위 타입이 상위 타입을 대체해도 프로그램이 정상 동작해야 한다는 원칙은?', c:['SRP','OCP','LSP','ISP'], a:2, e:'상속 관계에서 사전·사후 조건을 어기면 위반이다.' },
  { s:'s5', d:2, q:'레이어드 아키텍처의 안티패턴으로 거리가 먼 것은?', c:['컨트롤러에 비즈니스 로직 작성','엔티티를 그대로 API 응답으로 노출','서비스 간 무분별한 상호 호출','상위에서 하위로의 단방향 의존'], a:3, e:'단방향 의존은 지켜야 할 원칙이다.' },
  { s:'s5', d:3, q:'헥사고날(포트&어댑터) 아키텍처를 도입할 때 함께 말해야 할 점은?', c:['항상 도입하는 것이 옳다','규모가 작으면 과설계가 될 수 있다','JPA 를 쓸 수 없다','테스트가 어려워진다'], a:1, e:'도메인 격리로 얻는 이점과 구조 비용을 비교해 판단한다.' },
  { s:'s5', d:2, q:'애플리케이션이 캐시를 먼저 조회하고 없으면 DB 에서 읽어 채우는 전략은?', c:['Cache-Aside','Write-Through','Write-Behind','Refresh-Ahead'], a:0, e:'가장 흔한 방식이며 무효화 전략을 함께 설계해야 한다.' },
  { s:'s5', d:2, q:'2PC 대신 Saga 를 선택하는 주된 이유는?', c:['강한 일관성이 필요해서','가용성과 성능 손해가 커서','구현이 더 복잡해서','트랜잭션이 필요 없어서'], a:1, e:'Saga 는 최종 일관성을 받아들이는 대신 가용성을 지킨다.' },
  { s:'s5', d:2, q:'테스트에 대한 설명으로 가장 적절한 것은?', c:['E2E 테스트를 가장 많이 작성한다','커버리지 수치가 높으면 항상 좋은 테스트다','단위 테스트를 다수, E2E 는 소수로 둔다','테스트는 리팩터링 후에 작성한다'], a:2, e:'중요한 분기를 덮었는지가 수치보다 중요하다.' },
  { s:'s5', d:3, q:'DDD 관점에서 트랜잭션 경계를 잡는 기본 단위는?', c:['테이블','애그리거트','바운디드 컨텍스트','리포지터리'], a:1, e:'애그리거트가 일관성 경계이므로 그 단위로 묶는다.' },
  { s:'s5', d:2, q:'코드 리뷰에서 지적을 받았을 때 바람직한 태도는?', c:['즉시 모두 수용한다','의도를 확인하고 타당하면 수용, 이견은 근거로 제시한다','다음 리뷰까지 미룬다','리뷰어에게 설명을 요구하지 않는다'], a:1, e:'사람이 아니라 코드에 대한 의견으로 다룬다.' },

  // ───────── 보강 6. 인프라 · 데브옵스 ─────────
  { s:'s6', d:2, q:'Dockerfile 에서 빌드 캐시를 잘 활용하는 작성 순서는?', c:['소스 복사 후 의존성 설치','의존성 파일 먼저 복사·설치 후 소스 복사','모든 파일을 한 번에 복사','캐시는 순서와 무관하다'], a:1, e:'자주 바뀌는 레이어를 뒤에 두어야 캐시가 유지된다.' },
  { s:'s6', d:2, q:'멀티 스테이지 빌드의 주된 목적은?', c:['빌드 속도만 높인다','빌드 도구를 제외해 최종 이미지 크기를 줄인다','컨테이너 보안 그룹을 설정한다','로그를 분리한다'], a:1, e:'실행에는 JRE 슬림 이미지만 남긴다.' },
  { s:'s6', d:2, q:'소수 트래픽부터 점진적으로 확대하며 지표를 관측하는 배포 전략은?', c:['롤링','블루-그린','카나리','재생성'], a:2, e:'위험을 최소화하지만 관측 지표 설계가 전제다.' },
  { s:'s6', d:3, q:'Redis 분산 락을 구현할 때 반드시 포함해야 하는 요소는?', c:['만료 시간(PX)과 소유자 토큰 확인','KEYS 명령으로 락 조회','무한 대기','AOF 비활성화'], a:0, e:'만료가 없으면 장애 시 락이 영원히 남는다.' },
  { s:'s6', d:2, q:'AWS 에서 애플리케이션에 권한을 부여하는 올바른 방법은?', c:['액세스 키를 소스에 포함','액세스 키를 이미지에 포함','IAM 역할 부여','보안 그룹에 키 저장'], a:2, e:'키를 코드나 이미지에 넣으면 유출 사고로 직결된다.' },
  { s:'s6', d:2, q:'AWS 보안 그룹(SG)에 대한 설명으로 옳은 것은?', c:['Stateless 이며 거부 규칙을 쓴다','Stateful 이며 허용 규칙만 정의한다','서브넷 단위로만 동작한다','아웃바운드를 설정할 수 없다'], a:1, e:'Stateless 로 거부 규칙까지 쓰는 것은 NACL 이다.' },
  { s:'s6', d:1, q:'CI 가 제공하는 가장 큰 가치는?', c:['배포 자동화','커밋마다 빌드·테스트로 통합 위험 조기 발견','인프라 프로비저닝','모니터링'], a:1, e:'CD 는 그 이후의 자동 배포 단계다.' },

  // ───────── 보강 7. 자료구조 · 알고리즘 · CS ─────────
  { s:'s7', d:2, q:'간선이 적은 희소 그래프에서 일반적으로 선택하는 표현 방식은?', c:['인접 행렬','인접 리스트','해시 테이블','세그먼트 트리'], a:1, e:'인접 행렬은 간선 조회가 O(1)이지만 공간이 V제곱이다.' },
  { s:'s7', d:2, q:'자바에서 객체 배열을 정렬할 때 사용되는 알고리즘과 성질은?', c:['퀵소트 · 불안정','TimSort · 안정','힙소트 · 안정','버블소트 · 안정'], a:1, e:'기본형 배열은 듀얼 피벗 퀵소트를 사용한다.' },
  { s:'s7', d:3, q:'최댓값을 최소화하라는 유형에서 답 자체를 이분 탐색하는 기법은?', c:['투 포인터','파라메트릭 서치','슬라이딩 윈도우','분할 정복'], a:1, e:'판정 함수가 단조로울 때 성립한다.' },
  { s:'s7', d:2, q:'컨텍스트 스위칭 비용이 발생하는 주된 이유는?', c:['CPU 클럭이 낮아서','PCB 저장·복원과 캐시 무효화 때문','메모리가 부족해서','디스크 I/O 때문'], a:1, e:'스레드가 프로세스보다 전환 비용이 작다.' },
  { s:'s7', d:2, q:'뮤텍스와 세마포어의 차이로 옳은 것은?', c:['뮤텍스는 카운트를 가진다','세마포어는 소유권 개념이 핵심이다','뮤텍스는 소유권, 세마포어는 카운트 기반이다','둘은 완전히 같다'], a:2, e:'세마포어는 동시 접근 허용 개수를 제어한다.' },
  { s:'s7', d:1, q:'BFS 구현에 사용하는 자료구조는?', c:['스택','큐','힙','트리'], a:1, e:'DFS 는 스택 또는 재귀를 사용한다.' },

  // ───────── 보강 8. 인성 · 경험 면접 ─────────
  { s:'s8', d:2, q:'자기소개 답변으로 가장 바람직한 것은?', c:['이력서를 순서대로 낭독한다','1분 내로 경력 요약과 강점, 지원 동기를 연결한다','성장 배경부터 상세히 말한다','기술 용어를 최대한 많이 나열한다'], a:1, e:'무엇을 잘하는 사람인지 한 문장이 남아야 한다.' },
  { s:'s8', d:2, q:'약점을 묻는 질문에 대한 답변으로 가장 적절한 것은?', c:['완벽주의라 일을 오래 붙듭니다','약점이 없다고 답한다','실제 약점과 개선을 위해 하고 있는 노력을 말한다','업무와 무관한 성격을 말한다'], a:2, e:'위장형 약점은 준비 부족으로 읽힌다.' },
  { s:'s8', d:3, q:'일정이 촉박한데 품질을 지켜야 하는 상황에 대한 답변으로 가장 적절한 것은?', c:['무조건 일정을 맞춘다','범위·일정·품질 중 조정할 것을 이해관계자와 합의하고 부채를 명시한다','테스트를 생략한다','혼자 야근으로 해결한다'], a:1, e:'회수 계획까지 말하면 신뢰를 얻는다.' },
  { s:'s8', d:2, q:'희망 연봉을 말하는 방식으로 가장 적절한 것은?', c:['회사 규정에 따르겠다고만 답한다','구체적 범위를 제시하고 총보상 관점으로 논의한다','가능한 한 높은 단일 숫자를 부른다','면접에서는 언급하지 않는다'], a:1, e:'기준 없이 임하면 협상 여지가 사라진다.' },

  // ───────── 보강 5. 도메인 지식 (업종별) ─────────
  { s:'s5', d:2, q:'주문 금액을 상품 테이블과 조인해 매번 계산하도록 설계했을 때의 문제는?', c:['조회 성능만 떨어진다','상품 가격이 바뀌면 과거 주문 금액까지 바뀐다','인덱스를 못 탄다','트랜잭션이 길어진다'], a:1, e:'주문 시점 금액을 스냅샷으로 저장해야 한다.' },
  { s:'s5', d:3, q:'단일 컬럼 재고를 동시 차감할 때 가장 단순하면서 안전한 방법은?', c:['조회 후 애플리케이션에서 계산해 저장','UPDATE stock SET qty = qty - 1 WHERE id = ? AND qty > 0','SELECT 후 sleep 재시도','트랜잭션 격리 수준을 낮춘다'], a:1, e:'갱신된 행 수가 0이면 품절로 판단한다.' },
  { s:'s5', d:2, q:'결제에서 매입(Capture) 이후 금액을 되돌리는 처리는?', c:['승인 취소(Void)','환불(Refund)','망취소','정산'], a:1, e:'매입 전 승인 취소는 전액, 매입 후 환불은 부분도 가능하다.' },
  { s:'s5', d:3, q:'외부 결제 요청이 타임아웃되어 승인 여부를 알 수 없을 때 올바른 대응은?', c:['즉시 실패로 처리하고 재요청','미결 상태로 두고 조회 API 로 결과를 재확인','사용자에게 성공으로 안내','트랜잭션을 롤백하고 무시'], a:1, e:'멱등 키와 보정 배치를 함께 두어야 중복 결제를 막는다.' },
  { s:'s5', d:2, q:'정산(Settlement)과 대사(Reconciliation)의 차이로 옳은 것은?', c:['둘은 같은 말이다','정산은 지급액 계산·지급, 대사는 내부와 외부 내역 대조','정산은 배치, 대사는 실시간','대사는 회계 부서만 한다'], a:1, e:'대사 배치는 재실행해도 결과가 같아야 한다.' },
  { s:'s5', d:2, q:'잔액이나 재고를 컬럼 값 갱신으로만 관리할 때의 근본적인 문제는?', c:['성능이 느리다','사고 발생 시 추적·복구·설명이 불가능하다','인덱스를 못 쓴다','트랜잭션을 못 쓴다'], a:1, e:'거래 내역을 원장으로 쌓고 잔액을 파생시키는 설계가 안전하다.' },
  { s:'s5', d:2, q:'예약 시스템에서 결제 전 좌석을 잡아 두는 선점(hold) 설계에 반드시 필요한 것은?', c:['영구 잠금','만료 시간과 자동 해제','비관적 락 금지','좌석 삭제'], a:1, e:'(자원, 시간) 유니크 제약이 이중 예약의 최후 방어선이다.' },
  { s:'s5', d:3, q:'팔로워가 수백만인 사용자의 게시물에서 fan-out on write 방식의 문제는?', c:['읽기가 느려진다','쓰기 시 타임라인 반영량이 폭발한다','정합성이 깨진다','캐시를 못 쓴다'], a:1, e:'일반 사용자는 write, 인플루언서는 read 로 혼합하는 것이 실무 정석이다.' },
  { s:'s5', d:2, q:'구독 서비스에서 요금제를 중도 변경할 때 남은 기간을 일할 계산하는 개념은?', c:['던닝(Dunning)','프로레이션(Proration)','디퍼드(Deferred)','청산(Clearing)'], a:1, e:'던닝은 결제 실패 시 재시도·안내·해지 절차를 뜻한다.' },
  { s:'s5', d:2, q:'도메인 경험이 없는 회사에 지원했을 때 가장 바람직한 답변 전략은?', c:['경험이 없다고만 답한다','문제 구조가 같은 내 사례로 연결하고 학습 계획을 덧붙인다','아는 척 지어낸다','기술 스택 이야기로 돌린다'], a:1, e:'상태 머신·멱등성·정합성 검증은 업종이 달라도 통한다.' }
];
...
CPPG.ox = [
  ...
  // ───────── 보강 ─────────
  { s:'s1', d:1, q:'오버라이딩된 메서드는 런타임에 실제 객체 타입을 기준으로 호출된다.', a:true, e:'동적 바인딩이며 오버로딩은 컴파일 시점에 결정된다.' },
  { s:'s1', d:2, q:'인터페이스는 자바 8부터 default 메서드로 구현을 가질 수 있다.', a:true, e:'다중 구현이 가능해 무관한 타입에 능력을 부여할 때 쓴다.' },
  { s:'s1', d:2, q:'제네릭 타입 정보는 런타임에도 그대로 남아 있다.', a:false, e:'타입 소거로 지워지므로 new T() 같은 코드가 불가능하다.' },
  { s:'s1', d:2, q:'for-each 순회 중 컬렉션을 직접 수정하면 예외가 발생할 수 있다.', a:true, e:'ConcurrentModificationException 이며 Iterator.remove 로 해결한다.' },
  { s:'s1', d:1, q:'Metaspace 는 힙 안에 위치한다.', a:false, e:'네이티브 메모리에 위치한다.' },
  { s:'s1', d:2, q:'JIT 컴파일러는 자주 실행되는 바이트코드를 기계어로 변환한다.', a:true, e:'인터프리터의 반복 실행 비용을 줄인다.' },
  { s:'s1', d:2, q:'CAS 는 경합이 심할수록 재시도가 늘어 비용이 커질 수 있다.', a:true, e:'경합이 매우 심하면 락이 유리한 경우도 있다.' },
  { s:'s1', d:3, q:'orElse 에 전달한 값은 Optional 에 값이 있어도 평가된다.', a:true, e:'비용이 크면 orElseGet 을 사용한다.' },
  { s:'s1', d:2, q:'가상 스레드는 CPU 집약 연산의 처리 속도를 직접 높여 준다.', a:false, e:'I/O 대기가 많은 작업의 동시 처리량을 높이는 기능이다.' },
  { s:'s2', d:1, q:'DI 는 IoC 를 구현하는 방법 중 하나다.', a:true, e:'제어권을 컨테이너가 갖는 것이 IoC 다.' },
  { s:'s2', d:2, q:'프로토타입 스코프 빈은 컨테이너가 소멸 콜백까지 관리한다.', a:false, e:'생성 이후 관리는 사용하는 쪽 책임이다.' },
  { s:'s2', d:2, q:'@PostConstruct 는 의존관계 주입이 끝난 뒤 호출된다.', a:true, e:'생성 → 주입 → 초기화 콜백 순이다.' },
  { s:'s2', d:2, q:'스프링 부트 2.6 이상은 순환 참조를 기본적으로 허용한다.', a:false, e:'기본 금지이며 설계를 분리하는 것이 정답이다.' },
  { s:'s2', d:2, q:'@Cacheable 과 @Async 도 자기 호출에서는 동작하지 않는다.', a:true, e:'모두 프록시를 거쳐야 적용되는 기능이다.' },
  { s:'s2', d:2, q:'MANDATORY 는 트랜잭션이 없으면 새로 시작한다.', a:false, e:'없으면 예외를 던진다. 새로 시작하는 것은 REQUIRED 다.' },
  { s:'s2', d:2, q:'설정 값은 application.yml 이 환경 변수보다 우선한다.', a:false, e:'커맨드라인 인자 → 환경 변수 → 설정 파일 순이다.' },
  { s:'s2', d:2, q:'Actuator 엔드포인트는 운영에서 노출 범위를 제한해야 한다.', a:true, e:'인증 없이 열면 운영 정보가 노출된다.' },
  { s:'s2', d:2, q:'스프링 부트 3.x 는 javax 대신 jakarta 네임스페이스를 사용한다.', a:true, e:'Java 17 이상이 필수다.' },
  { s:'s3', d:2, q:'준영속 상태 엔티티의 값을 바꾸면 커밋 시 UPDATE 가 실행된다.', a:false, e:'변경 감지는 영속 상태에서만 동작한다.' },
  { s:'s3', d:3, q:'mappedBy 쪽 @OneToOne 은 지연 로딩이 동작하지 않는다.', a:true, e:'외래 키가 없어 존재 여부 확인이 필요하기 때문이다.' },
  { s:'s3', d:2, q:'IDENTITY 전략을 쓰면 persist 시점에 즉시 INSERT 가 실행된다.', a:true, e:'쓰기 지연 이점이 사라진다.' },
  { s:'s3', d:2, q:'엔티티에 롬복 @Data 를 붙이는 것은 권장된다.', a:false, e:'equals·hashCode·toString 문제로 지양한다.' },
  { s:'s3', d:2, q:'커버링 인덱스를 사용하면 테이블 접근을 생략할 수 있다.', a:true, e:'조회 컬럼이 모두 인덱스에 있을 때 가능하다.' },
  { s:'s3', d:2, q:'EXPLAIN 의 type 이 ALL 이면 풀 스캔을 의미한다.', a:true, e:'Using filesort·Using temporary 도 경고 신호다.' },
  { s:'s3', d:2, q:'카디널리티가 낮은 컬럼일수록 인덱스 효율이 좋다.', a:false, e:'선택도가 높은(값이 다양한) 컬럼이 유리하다.' },
  { s:'s3', d:3, q:'커넥션 풀 크기를 늘리면 언제나 처리량이 올라간다.', a:false, e:'DB 코어·디스크가 병목이면 경합만 늘어난다.' },
  { s:'s4', d:1, q:'UDP 는 연결을 수립하지 않아 DNS 나 실시간 전송에 쓰인다.', a:true, e:'TCP 는 연결·신뢰·순서·흐름 제어를 제공한다.' },
  { s:'s4', d:2, q:'HTTP/3 는 TCP 대신 QUIC(UDP) 을 사용한다.', a:true, e:'TCP 단의 HOL 블로킹을 해소한다.' },
  { s:'s4', d:2, q:'조건부 요청에서 리소스가 변경되지 않았으면 서버는 304 를 반환한다.', a:true, e:'ETag 나 Last-Modified 로 판단한다.' },
  { s:'s4', d:2, q:'SameSite 쿠키 속성은 XSS 를 직접 막아 준다.', a:false, e:'CSRF 완화 수단이며 XSS 는 출력 인코딩과 CSP 로 대응한다.' },
  { s:'s4', d:3, q:'OAuth 2.0 의 Implicit 플로우는 현재 권장되는 방식이다.', a:false, e:'Authorization Code + PKCE 가 권장된다.' },
  { s:'s4', d:2, q:'Spring Security 의 SecurityContextHolder 는 ThreadLocal 기반이다.', a:true, e:'스레드가 바뀌면 컨텍스트 전파에 주의해야 한다.' },
  { s:'s5', d:2, q:'개방-폐쇄 원칙은 확장에는 열려 있고 수정에는 닫혀 있어야 한다는 원칙이다.', a:true, e:'전략 패턴과 추상화가 대표적인 실현 수단이다.' },
  { s:'s5', d:2, q:'Cache-Aside 는 애플리케이션이 캐시 미스 시 DB 를 읽어 캐시를 채우는 방식이다.', a:true, e:'가장 흔한 전략이며 무효화 설계가 함께 필요하다.' },
  { s:'s5', d:2, q:'2PC 는 가용성과 성능 손해가 커서 실무 분산 환경에서 널리 쓰인다.', a:false, e:'그래서 Saga 와 최종 일관성을 택하는 경우가 많다.' },
  { s:'s5', d:3, q:'애그리거트는 일관성 경계이므로 트랜잭션 단위로 삼는 것이 기본이다.', a:true, e:'경계를 넘는 일관성은 이벤트로 맞춘다.' },
  { s:'s5', d:2, q:'테스트 커버리지 수치가 높으면 항상 좋은 테스트다.', a:false, e:'중요한 분기를 덮었는지가 더 중요하다.' },
  { s:'s6', d:2, q:'Dockerfile 은 자주 바뀌는 파일을 앞쪽에 복사해야 캐시가 잘 활용된다.', a:false, e:'자주 바뀌는 것을 뒤에 두어야 앞 레이어 캐시가 유지된다.' },
  { s:'s6', d:2, q:'멀티 스테이지 빌드는 최종 이미지에서 빌드 도구를 제외해 크기를 줄인다.', a:true, e:'실행 단계에는 JRE 슬림 이미지만 남긴다.' },
  { s:'s6', d:3, q:'Redis 분산 락은 만료 시간을 설정하지 않아도 안전하다.', a:false, e:'장애 시 락이 영원히 남아 서비스가 멈출 수 있다.' },
  { s:'s6', d:2, q:'AWS 보안 그룹은 Stateful 이며 허용 규칙만 정의한다.', a:true, e:'Stateless 이며 거부 규칙을 쓰는 것은 NACL 이다.' },
  { s:'s7', d:2, q:'자바에서 객체 정렬에 사용되는 TimSort 는 안정 정렬이다.', a:true, e:'기본형 배열은 듀얼 피벗 퀵소트를 사용한다.' },
  { s:'s7', d:2, q:'인접 행렬은 희소 그래프에서 메모리 효율이 좋다.', a:false, e:'정점 수의 제곱만큼 공간을 쓰므로 희소 그래프에는 리스트가 유리하다.' },
  { s:'s7', d:2, q:'세마포어는 카운트 기반이고 뮤텍스는 소유권 개념을 가진다.', a:true, e:'세마포어는 동시 접근 허용 개수를 제어한다.' },
  { s:'s8', d:2, q:'자기소개는 이력서를 순서대로 읽어 주는 것이 안전하다.', a:false, e:'1분 내로 강점과 지원 동기를 연결하는 편이 낫다.' },
  { s:'s8', d:2, q:'약점 질문에는 실제 약점과 개선 노력을 함께 말하는 것이 낫다.', a:true, e:'위장형 약점은 준비 부족으로 읽힌다.' },
  { s:'s8', d:2, q:'희망 연봉은 회사 규정에 따르겠다고만 답하는 것이 유리하다.', a:false, e:'기준 없이 임하면 협상 여지가 사라진다.' },
  { s:'s5', d:2, q:'주문 시점의 금액은 상품 테이블을 참조하지 말고 스냅샷으로 저장해야 한다.', a:true, e:'가격이 바뀌어도 과거 주문은 불변이어야 한다.' },
  { s:'s5', d:2, q:'매입(Capture) 이후에는 부분 환불이 가능하다.', a:true, e:'매입 전 승인 취소는 전액 단위로 이뤄진다.' },
  { s:'s5', d:3, q:'외부 결제 연동에 재시도만 추가해도 중복 결제는 발생하지 않는다.', a:false, e:'멱등 키와 상태 조회가 없으면 중복 결제가 난다.' },
  { s:'s5', d:2, q:'금액 계산에는 double 대신 정수형이나 BigDecimal 을 사용해야 한다.', a:true, e:'부동소수 오차가 정산 단계에서 드러난다.' },
  { s:'s5', d:2, q:'정산 배치는 같은 날짜로 두 번 실행하면 금액이 두 배가 되어도 정상이다.', a:false, e:'배치는 재실행해도 결과가 같아야(멱등) 복구가 가능하다.' },
  { s:'s5', d:2, q:'상태 값을 문자열로 두고 어디서나 변경할 수 있게 하면 불가능한 상태 전이가 발생할 수 있다.', a:true, e:'열거형과 허용 전이 규칙으로 통제해야 한다.' }
];
...
CPPG.fill = [
  ...
  // ───────── 보강 ─────────
  { s:'s1', d:1, q:'오버로딩과 오버라이딩의 결정 시점을 각각 쓰시오.', a:'오버로딩은 컴파일 시점(정적 바인딩), 오버라이딩은 런타임(동적 바인딩)', k:['컴파일','런타임'], e:'다형성의 근거가 되는 것은 오버라이딩이다.' },
  { s:'s1', d:2, q:'HashMap 의 기본 로드 팩터와 초과 시 동작을 쓰시오.', a:'0.75, 초과하면 테이블을 2배로 리사이즈하고 재해시', k:['0.75','리사이즈'], e:'리사이즈 비용까지 말하면 깊이가 드러난다.' },
  { s:'s1', d:2, q:'교착 상태의 4가지 조건을 쓰시오.', a:'상호배제, 점유와 대기, 비선점, 환형 대기', k:['상호배제','점유','비선점','환형'], e:'락 획득 순서를 통일하면 환형 대기를 깬다.' },
  { s:'s1', d:2, q:'클래스 로더의 위임 순서를 쓰시오.', a:'Bootstrap, Platform(Extension), Application 순으로 위임', k:['Bootstrap','Application'], e:'상위에 먼저 위임해 중복 로딩과 변조를 막는다.' },
  { s:'s1', d:1, q:'JVM 힙의 세대별 구조를 쓰시오.', a:'Young(Eden, Survivor 0/1)과 Old', k:['Eden','Survivor','Old'], e:'약한 세대 가설에 근거한 구조다.' },
  { s:'s1', d:2, q:'불변 객체를 만들기 위한 조건 3가지 이상을 쓰시오.', a:'final 클래스, 모든 필드 final, setter 없음, 가변 필드는 방어적 복사', k:['final','setter','방어적'], e:'스레드 안전과 캐싱 안전이라는 이점이 따라온다.' },
  { s:'s1', d:3, q:'제네릭 와일드카드 PECS 원칙을 풀어 쓰시오.', a:'읽기(생산자)는 extends, 쓰기(소비자)는 super', k:['extends','super'], e:'Producer-Extends, Consumer-Super 의 약자다.' },
  { s:'s2', d:2, q:'스프링 빈의 생명주기 순서를 쓰시오.', a:'생성, 의존관계 주입, @PostConstruct, 사용, @PreDestroy, 소멸', k:['주입','PostConstruct','PreDestroy'], e:'초기화 콜백은 주입이 끝난 뒤 호출된다.' },
  { s:'s2', d:3, q:'자기 호출로 @Transactional 이 동작하지 않을 때의 해결책 3가지를 쓰시오.', a:'다른 빈으로 분리, 자기 자신 주입, AopContext.currentProxy 사용', k:['분리','주입','AopContext'], e:'가장 권장되는 것은 책임을 다른 빈으로 분리하는 것이다.' },
  { s:'s2', d:2, q:'스프링 부트 설정 값의 우선순위를 높은 것부터 3단계 쓰시오.', a:'커맨드라인 인자, 환경 변수, 설정 파일(application.yml)', k:['커맨드라인','환경 변수','설정 파일'], e:'컨테이너 배포에서는 환경 변수 주입을 많이 쓴다.' },
  { s:'s2', d:2, q:'슬라이스 테스트 애너테이션 3가지와 각 범위를 쓰시오.', a:'@WebMvcTest 웹 계층, @DataJpaTest JPA 계층, @SpringBootTest 전체 컨텍스트', k:['WebMvcTest','DataJpaTest','SpringBootTest'], e:'범위를 좁혀야 빌드 시간이 유지된다.' },
  { s:'s2', d:2, q:'MANDATORY 와 NEVER 전파 속성의 동작을 각각 쓰시오.', a:'MANDATORY 는 트랜잭션이 없으면 예외, NEVER 는 있으면 예외', k:['없으면','있으면'], e:'경계를 강제하고 싶을 때 사용한다.' },
  { s:'s2', d:2, q:'순환 참조가 발생했을 때의 근본 해결 방향을 쓰시오.', a:'공통 로직 분리나 이벤트 기반 분리로 설계를 변경', k:['분리','설계'], e:'@Lazy 는 임시방편이며 부트 2.6+ 는 기본 금지다.' },
  { s:'s2', d:2, q:'스프링 부트 3.x 로 올릴 때 확인해야 할 변화 2가지를 쓰시오.', a:'Java 17 이상 필수, javax 에서 jakarta 로 네임스페이스 변경', k:['17','jakarta'], e:'의존 라이브러리 호환성도 함께 확인한다.' },
  { s:'s3', d:1, q:'JPA 엔티티의 생명주기 4가지 상태를 쓰시오.', a:'비영속, 영속, 준영속, 삭제', k:['비영속','영속','준영속','삭제'], e:'변경 감지는 영속 상태에서만 동작한다.' },
  { s:'s3', d:2, q:'페이징이 필요한 목록에서 컬렉션 연관을 조회할 때 권장되는 설정을 쓰시오.', a:'@BatchSize 또는 default_batch_fetch_size 로 IN 절 조회', k:['BatchSize','batch_fetch_size'], e:'ToOne 은 fetch join, 컬렉션은 배치가 정석 조합이다.' },
  { s:'s3', d:2, q:'커버링 인덱스가 무엇인지 쓰시오.', a:'조회 컬럼이 모두 인덱스에 있어 테이블 접근을 생략하는 인덱스', k:['테이블 접근','생략'], e:'조회 성능은 좋아지지만 인덱스 크기는 커진다.' },
  { s:'s3', d:2, q:'EXPLAIN 결과에서 성능 경고로 보아야 할 신호 2가지를 쓰시오.', a:'type 이 ALL(풀 스캔), Extra 의 Using filesort 또는 Using temporary', k:['ALL','filesort'], e:'추측 전에 실행 계획부터 확인한다.' },
  { s:'s3', d:2, q:'JPA 기본 키 생성 전략 3가지와 특징을 쓰시오.', a:'IDENTITY 는 즉시 INSERT, SEQUENCE 는 allocationSize 로 최적화, TABLE 은 느림', k:['IDENTITY','SEQUENCE','TABLE'], e:'MySQL 은 IDENTITY, Oracle 은 SEQUENCE 를 주로 쓴다.' },
  { s:'s3', d:2, q:'JPQL 과 QueryDSL 의 차이를 쓰시오.', a:'JPQL 은 문자열이라 컴파일 시 오류를 못 잡고, QueryDSL 은 타입 안전하며 동적 쿼리에 강함', k:['문자열','타입 안전'], e:'복잡한 튜닝 쿼리는 네이티브를 쓰기도 한다.' },
  { s:'s4', d:1, q:'TCP 3-way 핸드셰이크의 순서를 쓰시오.', a:'SYN, SYN+ACK, ACK', k:['SYN','ACK'], e:'종료는 FIN, ACK, FIN, ACK 의 4-way 다.' },
  { s:'s4', d:2, q:'HTTP/2 가 1.1 대비 개선한 점 3가지를 쓰시오.', a:'멀티플렉싱, 헤더 압축(HPACK), 바이너리 프레이밍', k:['멀티플렉싱','헤더 압축','바이너리'], e:'HTTP/3 는 QUIC 기반으로 TCP HOL 블로킹을 해소한다.' },
  { s:'s4', d:2, q:'쿠키의 보안 관련 속성 3가지와 역할을 쓰시오.', a:'HttpOnly 는 스크립트 접근 차단, Secure 는 HTTPS 전용, SameSite 는 교차 사이트 전송 제한', k:['HttpOnly','Secure','SameSite'], e:'SameSite 는 CSRF 완화 수단이다.' },
  { s:'s4', d:3, q:'모바일 앱이나 SPA 에서 권장되는 OAuth 2.0 플로우를 쓰시오.', a:'Authorization Code 방식에 PKCE 적용', k:['Authorization Code','PKCE'], e:'Implicit 은 폐기가 권장된다.' },
  { s:'s5', d:2, q:'전략 패턴을 적용해 해결하는 문제와 만족하는 원칙을 쓰시오.', a:'조건 분기 증가를 구현체 분리로 해결하며 개방-폐쇄 원칙을 만족', k:['분기','개방'], e:'결제 수단 추가가 대표적인 사례다.' },
  { s:'s5', d:2, q:'대표적인 캐시 전략 3가지를 쓰시오.', a:'Cache-Aside, Write-Through, Write-Behind', k:['Aside','Through','Behind'], e:'가장 흔한 것은 Cache-Aside 다.' },
  { s:'s5', d:3, q:'시스템 설계 면접의 진행 단계를 순서대로 쓰시오.', a:'요구사항 확인, 개략 추정, API와 데이터 모델, 개략 아키텍처, 병목과 확장, 트레이드오프 정리', k:['요구사항','추정','아키텍처','트레이드오프'], e:'요구사항 확인 없이 그림부터 그리면 감점이다.' },
  { s:'s5', d:2, q:'레이어드 아키텍처의 안티패턴 3가지를 쓰시오.', a:'컨트롤러에 비즈니스 로직, 서비스 간 무분별한 호출, 엔티티를 API 응답으로 노출', k:['컨트롤러','서비스','엔티티'], e:'의존은 상위에서 하위로 단방향이어야 한다.' },
  { s:'s6', d:1, q:'대표적인 배포 전략 3가지를 쓰시오.', a:'롤링, 블루-그린, 카나리', k:['롤링','블루','카나리'], e:'기능 플래그는 배포와 릴리스를 분리한다.' },
  { s:'s6', d:3, q:'Redis 분산 락 구현 시 반드시 포함해야 할 요소 2가지를 쓰시오.', a:'만료 시간 설정(SET NX PX), 소유자 토큰 확인 후 해제', k:['만료','소유자'], e:'짧은 임계 구간과 멱등 설계를 함께 말하면 좋다.' },
  { s:'s6', d:2, q:'RED 지표와 USE 지표의 구성 요소를 각각 쓰시오.', a:'RED 는 Rate, Errors, Duration / USE 는 Utilization, Saturation, Errors', k:['Rate','Duration','Utilization','Saturation'], e:'RED 는 요청 기반, USE 는 자원 기반 관점이다.' },
  { s:'s7', d:2, q:'입력 크기 n 이 1,000 과 10 일 때 각각 허용되는 대략적 복잡도를 쓰시오.', a:'n 이 1,000 이면 O(n제곱), n 이 10 이면 O(2의 n승) 또는 O(n!)', k:['제곱','2'], e:'제한 조건이 알고리즘을 사실상 지정한다.' },
  { s:'s7', d:2, q:'자바가 객체 배열과 기본형 배열 정렬에 사용하는 알고리즘을 각각 쓰시오.', a:'객체는 TimSort(안정), 기본형은 듀얼 피벗 퀵소트(비안정)', k:['TimSort','퀵소트'], e:'안정성이 필요한지가 선택 근거가 된다.' },
  { s:'s8', d:2, q:'면접 전에 준비해야 할 경험 5가지 유형을 쓰시오.', a:'성능 개선, 장애 해결, 기술 선택, 협업 갈등 조율, 실패와 배움', k:['성능','장애','기술 선택','협업','실패'], e:'각각을 STAR 로 정리해 소리 내어 연습한다.' },
  { s:'s8', d:2, q:'역질문을 준비할 때의 원칙과 피해야 할 유형을 쓰시오.', a:'면접 유형별로 2~3개 준비, 연봉과 복지만 묻거나 홈페이지에 있는 내용은 피함', k:['준비','연봉'], e:'없습니다 라는 답이 가장 나쁘다.' },
  { s:'s5', d:2, q:'결제 도메인에서 승인, 매입, 취소, 환불의 차이를 쓰시오.', a:'승인은 한도 확보, 매입은 청구 확정, 취소는 매입 전 전액 되돌림, 환불은 매입 후 부분 가능', k:['승인','매입','취소','환불'], e:'취소와 환불을 구분하지 못하면 도메인 이해가 얕아 보인다.' },
  { s:'s5', d:3, q:'업종과 무관하게 통하는 도메인 설계 문법 4가지 이상을 쓰시오.', a:'상태 머신, 멱등성, 원장과 이력, 정합성 검증(대사), 보상 트랜잭션', k:['상태','멱등','원장','대사'], e:'도메인 경험이 없을 때 이 구조로 답을 연결한다.' },
  { s:'s5', d:2, q:'한정 수량 상품의 재고 차감에서 사용할 수 있는 동시성 제어 수단 3가지를 쓰시오.', a:'DB 원자 UPDATE, 비관적 락, 낙관적 락과 재시도, 분산 락 또는 대기열', k:['원자','비관','낙관'], e:'충돌 빈도와 트래픽 형태로 선택 근거를 말한다.' },
  { s:'s5', d:2, q:'도메인 데이터에서 금액과 시간을 다루는 원칙을 각각 쓰시오.', a:'금액은 정수 또는 BigDecimal 로 double 금지, 시간은 UTC 로 저장하고 표시 시점에 변환', k:['BigDecimal','UTC'], e:'반올림 규칙과 마감 기준 시각도 함께 정의한다.' }
];
