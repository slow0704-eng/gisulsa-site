...
(function () {
  ...
  var PAGES = {
    ...
    write:
      ...
      '<p class="info-sub">I·II(정의·구조)는 "아는 것"의 증명일 뿐 — 다수가 같은 지점에 도달해 변별력이 없다. <b>합격선(마지막 1~2점)은 III(활용·전망)에서 갈린다.</b> 기존 22줄 골격 뒤에 대목차 <code>III.</code> + <b>간단 4줄 구성도</b>(표·첨언 없이)를 얹어 총 ≈27줄. 채점은 빠른 스캔이라 III는 <b>복잡할수록 감점</b> — 핵심만 남긴 4줄이 가독성·완결성에서 유리하다.</p>' +
      '<table><thead><tr><th>규칙</th><th>내용</th></tr></thead><tbody>' +
        '<tr><td><b>줄 수</b></td><td>대목차 <code>III. {토픽}의 활용 및 발전방향</code> + <b>4줄</b>(최소 3) · 박스[ ]+화살표(→ ↓ ⇄ ·)만 · <b>표·헤더선·첨언 없음</b></td></tr>' +
        '<tr><td><b>관점 라벨</b></td><td>각 줄 앞에 <code>적용: / 효과: / 고려: / 전망:</code> 라벨로 줄의 정체 명시 — 4줄이라 <code>↓</code> 최소화, <b>라벨만으로 자립</b></td></tr>' +
        '<tr><td><b>빈 화살표 금지</b></td><td>병렬 관점은 화살표 없이 <code>·</code> 나열 · 인과·적용·진화는 <code>→</code>(↓)에 <b>전이 라벨</b></td></tr>' +
        '<tr><td><b>비동어반복</b></td><td>I·II="무엇/어떻게" → III=<b>"어디에·무슨 이득·앞으로"</b> · 6-Key 중 <b>목적·트렌드·거버넌스</b>를 III에서 노출</td></tr>' +
        '<tr><td><b>폭 과시</b></td><td>마지막 줄에 <b>인접 토픽 1개</b> 연계(표준·차세대) · 미확인 수치는 <code>[확인필요]</code></td></tr>' +
      '</tbody></table>' +
      ...
      '<p><b>예시</b>(Zero Trust · 활용·효과형) — III 4줄:</p>' +
      '<pre class="diagram">적용: [금융·공공 망분리] → [원격근무 보안]\n효과: [횡적이동 차단] · [내부위협 노출면 최소화]\n고려: [레거시 통합 부담] → [단계적 PEP 배치]\n전망: [ZTNA → SASE·SSE 표준 수렴]</pre>' +
      '<div class="info-note">※ 기본 22줄(I·II)은 유지 — 3단락은 <b>고득점 강화 모드</b>(아는 토픽·차별화 필요 문항에 우선). 비교형(A vs B)엔 붙이지 않음(비교표 자체가 변별 요소). 통합사이트 카드 스키마는 불변 — 수기 답안 작성 시 얹는 변형. 상세: <code>답안/1교시_3단락_합격선_구성도_작성방안.txt</code>.</div>' +
